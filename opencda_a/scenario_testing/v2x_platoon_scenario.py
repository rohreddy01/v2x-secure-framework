# -*- coding: utf-8 -*-
"""
V2X Cooperative Platoon Scenario - Instance A
Run with:
    ...
    for msg in v2x_messages:
        ...
        for platoon in platoon_list:
            ...
            if intent == "emergency_stop":
                print(f"[V2X-A] EMERGENCY from Instance-{msg['instance']} — braking")
                ...
                if abs(current_speed - remote_speed) > 15:
                    ...
                    print(f"[V2X-A] Speed coord: {current_speed:.1f} -> {new_speed:.1f} km/h")
    v2x_messages.clear()
...
def run_scenario(opt, config_yaml):
    ...
    print(f"[V2X-A] Attack mode: {attack_type.upper()}  intensity={intensity}")

    # ── Set up injector and logger ───────────────────────────────────────────
    injector    = AttackInjector(attack_type=attack_type,
                                 intensity=intensity,
                                 flood_rate=flood_rate)
    data_logger = DataLogger(instance_id="A", attack_type=attack_type)

    scenario_params = config_yaml
    cav_world = CavWorld(opt.apply_ml)

    v2x_pub = V2XPublisher("A", injector=injector, data_logger=data_logger)
    v2x_sub = V2XSubscriber("A", on_v2x_receive)
    v2x_sub.start()
    print("[V2X-A] Bridge initialized")
    ...
    try:
        ...
        if 'current_time' not in scenario_params:
            scenario_params = OmegaConf.merge(
                scenario_params,
                OmegaConf.create({'current_time': opt.current_time})
            )
        ...
        print("\n[V2X-A] Simulation ended")

    finally:
        print("[V2X-A] Cleaning up...")
        v2x_pub.close()
        v2x_sub.stop()
        data_logger.close()
        if eval_manager:
